...
import { Zap, Heart, Timer, Trophy, RotateCcw, ChevronRight, Star, Brain, FlaskConical, Sigma } from 'lucide-react';
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-surface-100 p-5 shadow-sm space-y-3">
          ...
          <div className="space-y-2">
            {[
              { icon: '⏱️', text: '60 segundos por puzzle — ¡la rapidez suma!' },
              { icon: '❤️', text: '3 vidas en total — falla 3 veces y el juego termina' },
              { icon: '⚡', text: 'Respuesta correcta → +15–25 XP directos a tu cuenta' },
              { icon: '📖', text: 'Al fallar, verás la explicación didáctica antes de continuar' },
            ].map((r, i) => (
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-lg mx-auto">
        <div className="bg-white rounded-3xl border border-surface-100 shadow-xl overflow-hidden">
          <div className={`p-8 text-center bg-gradient-to-br ${perfect ? 'from-yellow-400 to-orange-500' : 'from-indigo-500 to-violet-600'}`}>
            <div className="text-6xl mb-3">{perfect ? '🏆' : correctCount >= 3 ? '🎯' : '💪'}</div>
      ...
      <div className="bg-white rounded-3xl border border-surface-100 shadow-lg overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-surface-400 italic">💡 Pista: {puzzle.hint}</p>
          </div>
            ...
            {phase === 'fail' && (
              ...
              >
                <div className="flex items-center gap-2 text-red-700 font-bold">
                  <span className="text-xl">{timedOut ? '⏰' : '❌'}</span>
