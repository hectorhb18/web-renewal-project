...
Termina con "3 acciones para subir de puntaje" en lista numerada.`;
    ...
    try {
      ...
      if (!res.ok || !data.text) {
        setError(
          data.error === 'quota_exceeded' ? 'Límite de solicitudes alcanzado, intenta en unos minutos.'
          : data.error === 'credits_exhausted' ? 'Se agotaron los créditos de IA del espacio de trabajo.'
          : 'No se pudo obtener el feedback. Intenta de nuevo.'
        );
        return;
      }
      ...
      setError('Error de conexión al enviar tu respuesta.');
    } finally {
      setLoading(false);
    }
  };
