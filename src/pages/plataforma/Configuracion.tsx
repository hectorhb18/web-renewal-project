...
import { Settings as SettingsIcon, Moon, Bell, Globe, Shield, Download, Trash2, Lock, Mail } from 'lucide-react';
import { loadState, updateSettings, exportData, deleteAccount } from '../../lib/store';
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-surface-500 text-sm mt-1">Personaliza tu experiencia en Studymind</p>
      </div>
...
function Section({ title, icon: Icon, children }: { title: string; icon: any; children: React.ReactNode }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      className="bg-white rounded-2xl border border-surface-100 shadow-sm p-6 space-y-3">
      <h3 className="text-sm font-bold text-surface-900 flex items-center gap-2 mb-2">
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-xl bg-surface-50">
      ...
      <button onClick={() => onChange(!value)}
        className={`relative w-12 h-6 rounded-full transition-colors flex-shrink-0 ${value ? 'bg-primary-500' : 'bg-surface-300'}`}>
...
function SelectRow({ label, hint, value, onChange, options, icon: Icon }: {
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-xl bg-surface-50 gap-4">
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="text-sm bg-white border border-surface-200 rounded-lg px-3 py-2 outline-none focus:border-primary-400">
