...
const TYPE_META: Record<Notification['type'], { icon: any; color: string; label: string }> = {
  tarea:        { icon: ClipboardList, color: 'bg-amber-100 text-amber-700',   label: 'Tarea' },
  recordatorio: { icon: Calendar,      color: 'bg-blue-100 text-blue-700',     label: 'Recordatorio' },
  ia:           { icon: Sparkles,      color: 'bg-primary-100 text-primary-700', label: 'Mensaje IA' },
  curso:        { icon: BookOpen,      color: 'bg-emerald-100 text-emerald-700', label: 'Curso' },
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        {unread > 0 && (
          <button onClick={() => { markAllNotificationsRead(); refresh(); }}
            className="text-xs font-semibold text-primary-600 hover:bg-primary-50 px-3 py-2 rounded-lg flex items-center gap-1.5">
        ...
        <AnimatePresence>
          {items.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <p className="text-sm text-surface-500">No hay notificaciones aquí.</p>
            </motion.div>
          ) : (
            items.map((n) => {
              ...
              return (
                <motion.button key={n.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  className={`w-full text-left bg-white rounded-2xl border p-4 flex gap-3 hover:border-primary-200 transition-all ${
                    n.read ? 'border-surface-100' : 'border-primary-200 shadow-sm'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-sm font-semibold text-surface-900 truncate">{n.title}</p>
                      {!n.read && <span className="w-2 h-2 rounded-full bg-primary-500 flex-shrink-0" />}
                    </div>
                    <p className="text-xs text-surface-600 line-clamp-2">{n.body}</p>
                    <p className="text-[11px] text-surface-400 mt-1.5">{timeAgo(n.createdAt)} · {meta.label}</p>
                  </div>
                </motion.button>
              );
            })
