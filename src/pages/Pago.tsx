...
import { BookOpen, Check, Crown, Zap, ShieldCheck, Lock, ArrowRight, Star, CreditCard, Sparkles } from 'lucide-react';
...
const plans = [
  {
    ...
    period: '/mes',
    badge: null,
    features: ['Acceso a todos los cursos', 'Práctica con IA', 'Recursos descargables', 'Soporte por chat'],
  ...
  },
  {
    ...
    features: ['Todo lo del plan Mensual', 'Tutores en vivo', 'Exámenes simulacro', 'Acceso offline', 'Certificados'],
  ...
  },
  {
    ...
    features: ['Todo lo del plan Semestral', 'Clases 1-a-1 mensuales', 'Acceso a vida', 'Prioridad en soporte'],
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-emerald-50 flex items-center justify-center px-4">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-surface-900 mb-3">¡Bienvenido, {name.split(' ')[0]}! 🎉</h2>
          <p className="text-surface-500 mb-8">Tu suscripción al plan <strong className="text-primary-600">{selectedPlan.name}</strong> está activa. Revisa tu correo en <span className="text-primary-600">{email}</span> para acceder.</p>
          <a
            href="#"
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-surface-500">
            Registrado como <span className="text-primary-600 font-semibold">{email}</span> · 14 días gratis incluidos
          </p>
        </div>
            ...
            <motion.div key="plan" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <div className="grid md:grid-cols-3 gap-6 mb-10">
                {plans.map((plan) => (
                  ...
                  >
                    {plan.badge && (
                      <span className={`absolute -top-3 left-1/2 -translate-x-1/2 whitespace-nowrap text-xs font-bold px-3 py-1 rounded-full ${
                        plan.id === 'anual' ? 'bg-amber-400 text-white' : 'bg-primary-600 text-white'
                      }`}>
                        {plan.badge}
                      </span>
                    )}

                    <div className={`w-5 h-5 rounded-full border-2 mb-4 flex items-center justify-center flex-shrink-0 ${
                      selected === plan.id ? 'border-primary-500 bg-primary-500' : 'border-surface-300'
                    }`}>
              ...
              <div className="text-center">
                ...
                <p className="text-surface-400 text-sm mt-4">Sin compromiso · Cancela en cualquier momento</p>
              </div>
            </motion.div>
          )}

          {tab === 'pago' && (
            <motion.div key="pago" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <div className="max-w-lg mx-auto">
                ...
                <div className="bg-white rounded-2xl border border-surface-100 shadow-sm p-6 mb-6">
                  ...
                  <div className="flex items-center justify-between text-sm mb-3">
                    ...
                    <span className="font-semibold text-surface-900">{selectedPlan.price}{selectedPlan.period}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm mb-3">
                  ...
                  <p className="text-xs text-surface-400 mt-2">Después del período de prueba se cobra {selectedPlan.price}{selectedPlan.period}</p>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-surface-100 shadow-sm p-6">
                  ...
                  <form onSubmit={handlePay} className="space-y-4">
                    <div>
                      <label className="block text-sm font-semibold text-surface-700 mb-2">Número de tarjeta</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold text-surface-700 mb-2">Vencimiento</label>
                        <input
                          type="text"
                          placeholder="MM/AA"
                          maxLength={5}
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-surface-700 mb-2">Nombre en la tarjeta</label>
                      <input
                        type="text"
                        defaultValue={name}
                  ...
                  <div className="mt-5 flex items-center justify-center gap-4 text-surface-400">
                    <div className="flex items-center gap-1 text-xs"><ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /> Pago seguro SSL</div>
                    <span>·</span>
                    <div className="flex items-center gap-1 text-xs"><Lock className="w-3 h-3 text-emerald-500" /> Datos encriptados</div>
                    <span>·</span>
                    <div className="flex items-center gap-1 text-xs"><Star className="w-3.5 h-3.5 text-amber-400" /> 45k+ alumnos</div>
                  </div>
                </div>
