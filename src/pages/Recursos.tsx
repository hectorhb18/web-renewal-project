...
export default function Recursos() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-surface-50 via-white to-primary-50/30 pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3.5 rounded-xl bg-white border border-surface-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 outline-none transition-all text-sm"
            />
          </div>
        </motion.div>
        ...
        <p className="text-sm text-surface-500 mb-6 text-center">
          <span className="font-semibold text-surface-700">{filtered.length}</span> recursos encontrados
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((res, i) => (
            <motion.a
              ...
              className={`group relative bg-white rounded-2xl p-6 border card-hover ${
                res.highlight ? 'border-primary-200 shadow-md shadow-primary-500/5' : 'border-surface-100'
              }`}
            >
              {res.highlight && (
                <div className="absolute -top-2.5 right-4">
                  <span className="bg-gradient-to-r from-primary-600 to-accent-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full">
                    ⭐ Recomendado
                  </span>
                </div>
              )}
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {res.free ? (
                    ...
                    <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-surface-100 text-surface-600">
                      Premium
                    </span>
                  )}
                </div>
              ...
              <h3 className="text-lg font-bold text-surface-900 group-hover:text-primary-600 transition-colors mb-2 flex items-center gap-2">
                {res.title}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-bold text-surface-900 mb-2">No se encontraron recursos</h3>
            <p className="text-surface-500">Prueba con otra búsqueda o categoría.</p>
          </div>
        )}
